...
const catalogSafeTop = 112
const catalogSafeBottom = 24

// 是否固定目录
function changePos() {
  let scrollTop = document.documentElement.scrollTop || document.body.scrollTop
  if (scrollTop > catalogTopHeight - 20) {
    catalog.style = `position: fixed; top: ${catalogSafeTop}px; bottom: ${catalogSafeBottom}px;`
  } else {
    catalog.style = `position: fixed; top: ${catalogSafeTop}px; bottom: ${catalogSafeBottom}px;`
  }
}
...
function isActiveCat() {
  // 可宽限高度值
  let offsetHeight = catalogSafeTop
  ...
  for(let i = 0; i < catLinkList.length; i++) {
    ...
    if (scrollTop >= currentTopCat && scrollTop < nextTopCat) {
      ...
      tocElement.scrollTop = Math.max(0, catLinkList[i].offsetTop - 32)
    } else {
...
function handleResize() {
  ...
  tocElement.setAttribute('style', `height: ${Math.max(220, windowHeight - catalogSafeTop - catalogSafeBottom - 36)}px`);
}
